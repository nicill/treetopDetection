...
def main(argv):
    ...
    if predict:
        ...
        if mosaic is None: raise Exception("forestSpeciesPatchClassifier, no mosaic read "+str(argv[6+offset]))
        roiFileName=argv[6+offset][:-4]+"ROI.jpg"
        outputDir=argv[7+offset]
    ...
    if training:
        ...
        print(conf)
        cm.computeMetrics(conf,data.classes)
    ...
    if(predict):
        ...
        alltops=listFromBinary(predictFileName,roiFileName) # now the prediction file is a binary file with treeTops marked
        predClassDict={}
        lostPoints=0
        randID=str(random.randint(0,5000))
        for cent in alltops:
            # classify t
            ...
            try:
                ...
                cv2.imwrite("./tempImage"+modelFileName+randID+".jpg",square)
                img=open_image("./tempImage"+modelFileName+randID+".jpg")
                pred_class,pred_idx,outputs =learn.predict(img)
                probability=float(outputs[pred_idx])
                if probability<probabilityThreshold: raise AssertionError("NO IDEA")
                #if str(pred_class) == "healthyfir": print("predicted "+str(pred_class)+" "+str(probability)+" "+str(outputs))
        ...
        #Santity check, we have the same number of points we had at the beggining
        predictedPoints=0
        for k,v in predClassDict.items():predictedPoints+=len(v)
        if predictedPoints+lostPoints<0.99*len(alltops):
            raise Exception("Predicted fewer tops than shold have "+str(predictedPoints+lostPoints)+" "+str(len(alltops)))
        else:
            print("predicted "+str(predictedPoints)+" lost "+str(lostPoints)+" total "+str(len(alltops)))
        ...
        for k,v in predClassDict.items():
            ...
            cv2.imwrite(outputDir+modelFileName+str(k)+"tops.jpg",maskImage)
