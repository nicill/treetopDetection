...
def execute_algorithm(image,min_hieght,result_file,result_image):
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5))
    time_start = time.perf_counter()
    ret,thresh = cv2.threshold(image,min_hieght,255,cv2.THRESH_TOZERO)
    im_erode = cv2.erode(thresh,kernel,iterations=1)
...
if __name__ == '__main__':
    ...
    min_tree_height = 2 #set minimun tree height
