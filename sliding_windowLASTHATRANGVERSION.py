...
def pureGradient(img):
    ...
    erosion=cv2.erode(img,erosionKernel,iterations=3)
...
def refineSeedsWithMaximums(inpImage,maskImage):
    ...
    for i in range(1,numLabels): #0 contains background
        auxMask[labelImage==i]=inpImage[labelImage==i]  # now auxMask is the mask of all the points in the current label
        i,j = np.unravel_index(auxMask.argmax(), auxMask.shape)  # now i,j contains the position of the max
        coordList.append((j,i))
        #erase the mask information
        auxMask[labelImage!=i]=0
    return coordList
"""
...
def eraseSmallRegions(im,oneTreePixels):
    ...
    for l in range(1,numLabels):
        ...
        if currentCount<oneTreePixels:
            count+=1
            retVal[labelImage==l]=0

    #print("erase small regions took out "+str(count)+" of "+str(numLabels))
    return retVal
"""
#lower=-1 means not lower, otherwise contains the value where to start sampling
def findTops(comp,args,minPix,maxPix,lower,realTopWindow=None): #receive a grayscale image of a connected component, threshold it repeatedly until you find all tree tops
    #pixMinConComp=int(args["minPixTop"])
    pixMinConComp=minPix

    #print("comparing the two! "+str(pixMinConComp)+" and argument "+str(int(args["minPixTop"])))
    ...
    if realTopWindow is not None:
        realNumPoints=len(CSE.listFromImage(realTopWindow))
        #print("and actually contains "+str(realNumPoints)+" With gradient perc "+str(gradientPixelPerc))
        if realNumPoints>0: print("```````````````````````NOT EMPTY "+str(realNumPoints)+" "+str(int(minTrees))+" "+str(int(maxTrees))+" "+str(gradientPixelPerc))
        else:print(str(realNumPoints)+" "+str(int(minTrees))+" "+str(int(maxTrees))+" "+str(gradientPixelPerc))

    #    aux=comp.copy()
    #    aux[realTopWindow==0]=255
        #cv2.imwrite("compREAL"+str(realNumPoints)+"MIN"+str(int(minTrees))+"MAX"+str(int(maxTrees))+"gradPixPerc"+str(int(gradientPixelPerc))+".jpg",aux)
        #cv2.imwrite("TOPScompREAL"+str(realNumPoints)+"MIN"+str(int(minTrees))+"MAX"+str(int(maxTrees))+"gradPixPerc"+str(int(gradientPixelPerc))+".jpg",realTopWindow)
...
#        if realTopWindow is not None:
#            if len(tops)>=realNumPoints:
                #print("***********************************REALLY finished tops in "+str(numIterations-1)+" grad "+str(int(gradientPixelPerc))+" "+str(realNumPoints)+" list "+str(numberTopsFoundList)+"\n\n\n")
#                print(str(realNumPoints)+" "+str(gradientPixelPerc)+" "+str(numIterations-2)+" "+str(demNumSteps))
#                return tops
    ...
    #print("finished tops, in "+str(numIterations)+" finding "+str(len(tops))+" "+str(numberTopsFoundList)+"\n\n\n")
    #if realTopWindow is not None:
    #    print(str(realNumPoints)+" "+str(gradientPixelPerc)+" "+str(numIterations-2)+" "+str(demNumSteps)+" NOFIN")
...
def processWindow(win,args,minNumPointsTree,maxNumPointsTree,isLower=-1,realTopWindow=None):
...
#        cv2.imwrite("binarised.png",win)
    ...
    binarized=win.copy()
    binarized[win<0.01]=0
    binarized[win>=0.01]=255

    #cv2.imwrite("binarised.jpg",binarized)
    ...
    for l in range(1,numLabels):
        ...
        if np.sum(thisComponent!=0)>minNumPointsTree:
            if realTopWindow is not None: realTopWindow[labelImage!=l]=255
            thisCompTops=findTops(thisComponent,args,minNumPointsTree,maxNumPointsTree,isLower,realTopWindow)
...
def main():
    ...
    ap.add_argument("-tFile", "--topfile", required=False, help="Path of the top ground truth file")
    ...
    minNumPointsTopLower=int(2*float(args["minPixTop"]))

    maxNumPointsTree=195000
    lowerPercent=25
    ...
    #cv2.imwrite("./merda.jpg",(secondBand*255/maxDem).astype("uint8"))

    #print(" shape of the bands!!!!"+str(firstBand.shape)+" "+str(secondBand.shape))
    ...
    if args["topfile"] is not None:
        realTops=cv2.imread(args["topfile"],cv2.IMREAD_GRAYSCALE)

    for (x, y, window) in sliding_window(firstBand, stepSize=int(args["size"])-100, windowSize=(int(args["size"]), int(args["size"])),allImage=False):
        #print("window "+str(x)+" "+str(y))
        if args["topfile"] is not None:
            realTopWindow=realTops[y:y + int(args["size"]), x:x + int(args["size"])]
        else: realTopWindow=None

        thisWindowSeeds=[]
        if np.sum(window>0)>minNumPointsTree:
            #print("                     going to process window ")
            thisWindowSeeds=processWindow(window,args,minNumPointsTop,maxNumPointsTree,-1,realTopWindow)
        #else:
        #    print("                     NOT PROCESSING window "+str(np.sum(window>0))+" vs "+str(minNumPointsTree))
        ...
        if np.sum(lowerWindow>0)>minNumPointsTreeLower:
            ...
            thisWindowLowerSeeds=processWindow(lowerWindow,args,minNumPointsTopLower,maxNumPointsTree,cutOff,realTopWindow)
